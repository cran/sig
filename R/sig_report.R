...
#' @return An object of class ``sig.report'' with the elements.
#' \itemize{
...
#'   \item{fns_with_many_args}{Names of each function with more args than 
#'   \code{too_many_args}.}
...
#' }
#' @details
#' \code{sig_report} summarises the number of input arguments and the number
#' of lines of each function in an environment of file, and identifies problem
#' files, in order to help you refactor your code.
...
#' The number of lines of code that a function takes up is subjective in R;
#' this function uses \code{length(deparse(fn))}. 
#' @examples
...
#' @rdname sig_report               
#' @method print sig.report
#' @export
print.sig.report <- function(x, ...)
{
  with(x, cat(
  ...
  print(x$n_args, ...)
  cat("These functions have more than", x$too_many_args, "input args:\n")
  print(noquote(x$fns_with_many_args), ...)
  cat("Distribution of the number of lines of the functions:") 
  print(x$n_lines, ...)                                                 
  cat("These functions have more than", x$too_many_lines, "lines:\n")
